...
def inverse_kinematic(
    ...
    size = build_jacobian(joints, list(current_guess)).shape[1]
    lambda_i = lambda_constant * np.identity(size)
    ...
    for _ in range(max_steps):
        ...
        delta_q = - mm(np.linalg.inv(mm(j.T,j) + lambda_i), mm(j.T, error)) #type: ignore 
...
def test_inverse_kinematics():
    ...
    inverse_kinematic(joints, position, orientation, initial_guess, visualize=True)
